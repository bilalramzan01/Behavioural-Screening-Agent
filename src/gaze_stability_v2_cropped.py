...
#
# IMG_5033.MOV INVESTIGATION LOG (62.3% on-camera vs ~90% typical, visually
# confirmed the person was looking at the camera almost the whole time):
#
# Attempt 1 (REVERTED): hypothesized glasses-lens glare fragments the
# pupil's dark region, so picked the single largest dark contour instead
# of averaging over the whole thresholded region. This regressed EVERY
# video, not just glasses ones - picking "largest contour" is unstable
# frame-to-frame (can jump between pupil/eyebrow/lash blobs). Diagnostic
# logging also showed the glasses video's blob-confidence was statistically
# indistinguishable from a clean video's, ruling out that hypothesis.
#
# Attempt 2 (CURRENT): visually inspected annotated flagged frames
# (red dot = computed pupil position) side by side with normal frames.
# Found the real pattern: EYE_CASCADE gives inconsistent box sizes
# frame-to-frame for this person - sometimes tight around just the eye,
# sometimes ballooning upward to include the eyebrow. Because
# locate_pupil_center() averages over the WHOLE box, when the box
# balloons upward the centroid gets pulled toward the dark eyebrow,
# producing a large fake vertical deviation even though the eye itself
# never moved. This is a box-instability problem, not a glare problem.
#
# Fix: exclude the top ~30% of each detected eye box before running
# darkness detection - that's the region most likely to be eyebrow when
# the box overshoots, and a pupil is essentially never located there.
# Pupil position is then mapped back into the ORIGINAL box's coordinate
# system so h_ratio/v_ratio definitions (and therefore DEVIATION_THRESHOLD)
# stay comparable to before.
...
EYEBROW_CROP_FRAC = 0.30     # fraction of eye box height to exclude from the TOP before
                              # darkness detection, to avoid the eyebrow-contamination bug
...
def locate_pupil_center(eye_gray):
    ...
    Returns coordinates relative to eye_gray as passed in (caller is
    responsible for offsetting back to the original box if eye_gray was
    itself a crop of a larger box).
    """
...
def locate_pupil_center_cropped(eye_gray, crop_frac=EYEBROW_CROP_FRAC):
    """
    Same as locate_pupil_center(), but first excludes the top crop_frac of
    the eye box before searching for the dark pupil region - this is the
    zone most likely to be eyebrow when EYE_CASCADE's box overshoots
    upward, and a pupil is essentially never located there.

    Returns (cx, cy) mapped back to the ORIGINAL (uncropped) eye_gray's
    coordinate system, so callers can keep using cx / ew and cy / eh
    exactly as before.
    """
    h, w = eye_gray.shape
    ...
    if search_region.size == 0:
        return w / 2, h / 2  # degenerate box, fall back to full-box center

    cx, cy_in_crop = locate_pupil_center(search_region)
    cy = cy_in_crop + crop_rows  # map back to original box coordinates
    return cx, cy
