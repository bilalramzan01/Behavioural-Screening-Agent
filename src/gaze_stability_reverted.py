...
#
# NOTE on the glasses investigation (IMG_5033.MOV, 62.3% on-camera vs.
# ~90% typical, visually confirmed the person was looking at the camera
# almost the whole time):
#
# A first attempted fix swapped locate_pupil_center()'s centroid
# calculation from "moments over the WHOLE thresholded dark region" to
# "moments over just the SINGLE LARGEST dark contour", on the theory that
# glasses glare fragments the pupil and picking the biggest solid blob
# would be more robust. That change was reverted: it regressed EVERY
# video (not just glasses ones), most likely because which blob is
# "largest" can flip between frames (pupil vs. an eyebrow/lash/shadow
# blob), losing the frame-to-frame continuity that the original
# whole-region average had. Diagnostic logging also showed the glasses
# video's darkness-blob confidence was statistically indistinguishable
# from the clean video's - so glare/blob-fragmentation was not, in fact,
# what's causing IMG_5033's low score. That hypothesis is currently
# considered ruled out, not fixed.
#
# Current status: root cause of IMG_5033's discrepancy is still
# UNCONFIRMED. Next step is visual: overlay the computed pupil point on
# the actual flagged frames (see visualize_flagged_frames.py) and look
# at what's really happening geometrically, rather than hypothesizing
# further.
...
def locate_pupil_center(eye_gray):
    ...
    ORIGINAL, PROVEN VERSION - averages over the whole thresholded dark
    region rather than picking a single "largest" contour. Do not swap
    this for single-contour selection again without re-validating across
    the full batch first - see module docstring above for why.
    """
