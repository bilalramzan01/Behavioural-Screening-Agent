...
# video's own baseline gaze position AND its own noise floor (since both
# the "center" position and the amount of natural micro-movement vary by
# person/camera angle - neither a fixed absolute position nor a fixed
# absolute deviation threshold generalizes across different videos).
#
...
#
# 1. (REVERTED) Hypothesized glasses-lens glare fragments the pupil's dark
#    region; picked the single largest dark contour instead of averaging
#    over the whole region. Regressed EVERY video - "largest contour" is
#    frame-to-frame unstable. Diagnostic logging also showed no real
#    confidence difference between glasses and non-glasses videos, ruling
#    this hypothesis out.
#
# 2. (KEPT) Visual inspection of annotated frames found the real bug:
#    EYE_CASCADE gives inconsistent box sizes frame-to-frame, sometimes
#    ballooning upward to include the eyebrow. Averaging over the whole
#    box then let dark eyebrow pixels pull the centroid up, producing
#    fake vertical deviation with no real eye movement. Fix: exclude the
#    top 30% of each eye box (EYEBROW_CROP_FRAC) before searching for the
#    dark pupil region. Took IMG_5033 to 88.4%, held or improved most
#    other videos.
#
# 3. (KEPT) Inspecting IMG_5033's post-fix deviation distribution showed
#    it decays smoothly with no clean gap between "noise" and "real
#    deviation" - and the old fixed DEVIATION_THRESHOLD=0.08 happened to
#    sit almost exactly at this video's own 90th percentile, suggesting
#    0.08 was never a principled cutoff, just whatever flagged a
#    plausible-looking fraction of frames on whichever video it was
#    first tuned on. Replaced the fixed threshold with a PER-VIDEO
#    adaptive one: flag frames beyond median + K_MAD * MAD (median
#    absolute deviation, scaled to behave like a standard deviation).
#    This adapts to each video's own noise floor instead of assuming
#    a single constant means the same thing for everyone - important
#    given this score feeds a real behavioral assessment across
#    different applicants.
#
# Still open: two genuine outlier frames in IMG_5033 (~2-3x every other
# frame's deviation) were flagged for one-off visual confirmation before
# fully trusting this - see extract_specific_frames.py. If they turn out
# to be real glances away, that's the system working correctly, not a bug.
...
EYEBROW_CROP_FRAC = 0.30     # exclude top X% of each eye box before darkness detection
                              # (avoids the eyebrow-contamination bug - see log above)
K_MAD = 3.0                  # adaptive threshold = median + K_MAD * scaled_MAD
                              # 3.0 is a standard robust-outlier convention (~3-sigma
                              # equivalent), chosen so isolated large deviations get
                              # flagged without over-flagging ordinary micro-movement

# scale factor so MAD approximates a standard deviation under a roughly normal
# distribution (the standard 1.4826 constant); makes K_MAD comparable to a
# familiar "sigma" multiplier
MAD_SCALE = 1.4826
...
def locate_pupil_center(eye_gray):
    ...
    centroid. Averages over the whole thresholded dark region (proven more
    stable than picking a single "largest" contour - see log above).
    """
...
def locate_pupil_center_cropped(eye_gray, crop_frac=EYEBROW_CROP_FRAC):
    ...
    eye box first - that's the zone most likely to be eyebrow when
    EYE_CASCADE's box overshoots upward, and a pupil is essentially never
    located there. Returns (cx, cy) mapped back to the ORIGINAL box's
    coordinate system.
    """
    h, w = eye_gray.shape
...
def analyze_gaze_stability(video_path: str, k_mad: float = K_MAD) -> dict:
    ...
    MAD of that video's own deviation values) rather than a fixed constant,
    since natural micro-movement noise varies person to person - see log
    above for why a fixed threshold was replaced.
    ...
    # Calibrate position against this video's OWN baseline (median) - "center"
    # varies by camera angle and individual eye geometry.
    baseline_h = np.median(h_ratios)
    baseline_v = np.median(v_ratios)
    deviation = np.sqrt((h_ratios - baseline_h) ** 2 + (v_ratios - baseline_v) ** 2)

    # Calibrate the FLAGGING THRESHOLD against this video's own noise floor too,
    # using median + k*MAD (robust to the outliers we're trying to detect,
    # unlike mean/std which the outliers themselves would inflate).
